...
- **All Edge Config reads must pass `consistentRead: true`.** On Vercel, `@vercel/edge-config` reads a filesystem snapshot at `/opt/edge-config/<id>.json` and memoises the parsed result in a module-level `Map` keyed **only by Edge Config id** — it never re-parses for the life of the process. Under Fluid Compute, instances stay warm, so a warm function can serve a frozen pre-write snapshot indefinitely. After an admin save, `updateTag` + `revalidatePath` correctly trigger a re-render, but that re-render read the stale snapshot and baked it into a `cacheLife("max")` entry — so each save appeared to land one save late, or not at all. `consistentRead: true` skips the snapshot and the parse cache and hits the origin. The SDK docs explicitly endorse it for "reading Edge Config specifically for generating a page using ISR", which is exactly what `app/lib/{projects,content,feature-gates}.ts` do — the read only happens on a cache miss, so the extra latency is paid once per invalidation, not per request. `readProjectsForAdmin` needs it too: the form posts the whole array back, so loading a stale snapshot and saving would silently revert newer edits. Still present as of `@vercel/edge-config` 1.5.1.
